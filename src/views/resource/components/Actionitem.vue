...
<template>
  ...
  >
    <template
      v-for="(item, index) in props.actionlist"
      :key="item.id"
      style="width: 22%"
    ...
    >
      <div v-if="item.id !== '-1'" class="flex px-5 my-5 justify-center flex-col items-center">
        <div v-if="item.picurl" class="w-[100px] h-[100px] mt-[10px] overflow-hidden">
          <img :src="item.picurl[0].URL" class="w-full h-full object-cover" />
        </div>
        <el-image
          v-else
          class="w-[100px] h-[100px] mt-[10px] object-cover"
        ...
        <div class="mt-2 w-full flex flex-col justify-around items-center content-center">
          ...
          <div class="flex w- flex-row mt-1">
            <BaseButton @click="action('edit', item, index)" type="primary"> 编辑 </BaseButton>
            <BaseButton @click="deteleAction(item)" type="danger"> 删除 </BaseButton>
          </div>
        </div>
      </div>
    </template>
    ...
    <div
      @click="action('add')"
      class="flex justify-center items-center flex-col content-center"
      style="
        width: 170px;
        height: 170px;
        border-width: 1px;
    ...
    <ActionDetail
      ref="actionDetailRef"
      @close-dialog="updataActionlist(props.SecondCategoryID)"
      :secondCategoryId="props.SecondCategoryID"
      :actionId="actionId"
